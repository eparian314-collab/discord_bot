...
class AdminCog(commands.Cog):
    ...
    async def mute_user(
        ...
        if member == interaction.user:
            await interaction.followup.send("❌ You can't mute yourself!", ephemeral=True)
            return
        
        if member.top_role >= interaction.user.top_role and interaction.user != interaction.guild.owner:
            await interaction.followup.send("❌ You can't mute someone with equal or higher role!", ephemeral=True)
            return
