...
class RankingCog(commands.Cog):
    ...
    @ranking.command(
        name="results",
        description="View your KVK results for a specific run"
    )
    @app_commands.describe(
        run="KVK run number (starting at 1)",
        day="Optional day (1-6; 6 represents the war stage total)"
    )
    async def ranking_results(self, interaction: discord.Interaction, run: int, day: Optional[int] = None):
    ...
    @ranking.command(
        name="compare_me",
        description="Compare your performance between two KVK runs"
    )
    @app_commands.describe(
    ...
    @ranking.command(
        name="compare_others",
        description="Compare your KVK results against peers or a friend"
    )
    @app_commands.describe(
        run="KVK run number",
        day="Optional day (1-6; omit for overall run)",
