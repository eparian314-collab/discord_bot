...
- `docs/` – detailed walkthroughs (`RANKING_SYSTEM.md`, `kvk_tracker_diagnostic.instructions.md`, setup checklists, admin guides).
...
**Overview** – Bot automatically cleans up its old messages from previous sessions on startup to maintain clean channels.

- **Session Tracking**
  - Each bot restart creates a new session with unique UUID
  - Session timestamps stored in `data/session_state.json`
  - Previous session time used as cleanup cutoff point

- **Automatic Cleanup (on startup)**
  - Deletes bot's own messages from before current session
  - Respects rate limits (0.5s delay between deletions)
  - Skips pinned messages and messages with "DO NOT DELETE" keywords
  - Skips recent messages (default: last 30 minutes)
  - Skips channels in blocklist (bot-logs, announcements, etc.)

- **Manual Cleanup Command**
  - `/admin cleanup [limit]` - Admins can trigger cleanup in current channel
  - Limit: 1-500 messages to check (default: 100)
  - Same safety rules apply (pins, keywords, etc.)

- **Configuration (.env)**
  ```bash
  CLEANUP_ENABLED=true                # Enable/disable cleanup
  CLEANUP_SKIP_RECENT_MINUTES=30      # Don't delete messages newer than this
  CLEANUP_LIMIT_PER_CHANNEL=200       # Max messages to check per channel
  CLEANUP_RATE_DELAY=0.5              # Seconds between deletions
  ```

- **Key Files**
  - `core/engines/session_manager.py` - Session tracking and persistence
  - `core/engines/cleanup_engine.py` - Message deletion logic and safety filters
  - Integration in `integrations/integration_loader.py` (on_ready event)
  - Manual command in `cogs/admin_cog.py` (/admin cleanup)
...
- **Submission Flow**
  1. `/ranking_submit` available only in `RANKINGS_CHANNEL_ID`.
  ...
  5. Admins can use `/ranking_view`, `/ranking_leaderboard`, `/ranking_report`, `/ranking_stats`, `/ranking_user`.
...
- Apply `/ranking_leaderboard` and `/ranking_stats` on a test guild to verify modlog + admin flows.
...
- KVK tracker diagnostic instructions: `docs/kvk_tracker_diagnostic.instructions.md`.
